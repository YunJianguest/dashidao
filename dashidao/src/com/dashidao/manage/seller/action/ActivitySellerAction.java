...
public class ActivitySellerAction {
    ...
    public void activity_goods(HttpServletRequest request, HttpServletResponse response, String goods_name){
        ...
        List<Goods> goods_list = this.goodsService
                                 .query(
                                     "select obj from Goods obj where obj.goods_name like :goods_name and obj.goods_status=:goods_status and obj.group_buy =:group_buy and obj.activity_status =:activity_status and obj.delivery_status=:delivery_status and obj.combin_status=:combin_status order by obj.addTime desc",
                                     params, -1, -1);
                ...
                for (String id : ids){
                    if (!id.equals("")){
                        ...
                        ag.setAg_price(num.multiply(act.getAc_rebate())
                                       .multiply(goods.getGoods_price()));
                        this.activityGoodsService.save(ag);
                    }
                }
                mv.addObject("op_title", "申请参加活动成功");
                mv.addObject("url", url +
                             "/seller/activity_goods_list.htm?id="+act_id);
            }else{
    ...
    public ModelAndView activity_goods_list(HttpServletRequest request, HttpServletResponse response, String currentPage,String id){
        ModelAndView mv = new JModelAndView(
        ...
        qo.addQuery("obj.act.id",new SysMap("act_id",Long.parseLong(id)), "=");
