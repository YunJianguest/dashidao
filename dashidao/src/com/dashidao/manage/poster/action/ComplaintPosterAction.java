...
import com.dashidao.foundation.domain.query.ComplaintQueryObject;
import com.dashidao.foundation.service.IAccessoryService;
import com.dashidao.foundation.service.IComplaintService;
import com.dashidao.foundation.service.IComplaintSubjectService;
...
@Controller
public class ComplaintPosterAction {
    @Autowired
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "栈代投诉发起", value = "/poster/complaint_handle.htm*", rtype = "poster", rname = "投诉管理", rcode = "complaint_poster", rgroup = "客户服务")
    @RequestMapping({"/poster/complaint_handle.htm"})
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "栈代被投诉列表", value = "/poster/complaint.htm*", rtype = "poster", rname = "投诉管理", rcode = "complaint_poster", rgroup = "客户服务")
    @RequestMapping({"/poster/complaint.htm"})
    public ModelAndView complaint_poster(HttpServletRequest request, HttpServletResponse response, String currentPage, String status){
        ModelAndView mv = new JModelAndView(
            "user/default/usercenter/poster_complaint.html", this.configService
            .getSysConfig(),
            this.userConfigService.getUserConfig(), 0, request, response);
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "栈代查看投诉详情", value = "/poster/complaint_view.htm*", rtype = "poster", rname = "投诉管理", rcode = "complaint_poster", rgroup = "客户服务")
    @RequestMapping({"/poster/complaint_view.htm"})
    public ModelAndView complaint_view(HttpServletRequest request, HttpServletResponse response, String id){
        ModelAndView mv = new JModelAndView(
            "user/default/usercenter/poster_complaint_view.html",
        ...
        if ((obj.getFrom_user().getId().equals(
            ...
            mv.addObject("url", CommUtil.getURL(request) +
                         "/poster/complaint.htm");
        }

        return mv;
    }

    @SecurityMapping(display = false, rsequence = 0, title = "栈代查看投诉详情", value = "/poster/complaint_appeal.htm*", rtype = "poster", rname = "投诉管理", rcode = "complaint_poster", rgroup = "客户服务")
    @RequestMapping({"/poster/complaint_appeal.htm"})
    public ModelAndView complaint_appeal(HttpServletRequest request, HttpServletResponse response, String id, String to_user_content){
        ...
        mv.addObject("url", CommUtil.getURL(request) + "/poster/complaint.htm");
