...
import com.dashidao.core.tools.StringUtils;
...
public class UserViewAction {
    ...
    @ResponseBody
    public void register_save(HttpServletRequest request, HttpServletResponse response,String id) throws IOException 
    {
    	Map params = new HashMap();
    	params.put("state", "0");
    	/*Claims claims = SignFilter.checkSign(request, response);
    	if (claims != null) {
    		...
    		recommend.setRecommend_name(telephone);
    		recommend.setAddTime(new Date());
    		if(type_tj.equals("1")){
    			recommend.setRecommend_type("0");
    		}else if(type_tj.equals("0")){
    			recommend.setRecommend_type("1");
    		}
