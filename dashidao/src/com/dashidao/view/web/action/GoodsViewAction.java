...
import com.dashidao.view.web.tools.IpAddress;
...
public class GoodsViewAction {
     ...
     * @throws IOException
     */
    @RequestMapping({ "/wap/tuiJianGoods_list.htm" })
    @ResponseBody
    public void tuiJianGoods_list(HttpServletRequest request, HttpServletResponse response,String currentPage) throws IOException 
    {
    	Map params = new HashMap();
    	Claims claims = SignFilter.checkSign(request, response);
    	if (claims != null) {
			String id = claims.getId();
			GoodsQueryObject qo = new GoodsQueryObject(currentPage, params,"addTime", "desc");
			qo.addQuery("obj.goods_zt", new SysMap("goods_zt",1), "=");
