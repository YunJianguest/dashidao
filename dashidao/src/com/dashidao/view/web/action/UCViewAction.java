...
public class UCViewAction {
    ...
    private String uc_answer(HttpServletRequest request, HttpServletResponse response){
        ...
        if (action.equals("synlogin")){
            if (!API_SYNLOGIN)
                return API_RETURN_FORBIDDEN;
            dashidao_login(request, response, get);
        }else if (action.equals("synlogout")){
            if (!API_SYNLOGOUT)
                return API_RETURN_FORBIDDEN;
            dashidao_logout(request, response, get);
        }else{
            ...
            if (action.equals("updatepw")){
                if (!API_UPDATEPW)
                    return API_RETURN_FORBIDDEN;
                dashidao_update_pws(request, response, get);
                return API_RETURN_SUCCEED;
            }
    ...
    protected void dashidao_login(HttpServletRequest request, HttpServletResponse response, Map<String, String> args){
        ...
        if (!admin_login){
            ...
            String url = CommUtil.getURL(request) +
                         ...
                         "dashidao_thid_login_" + password + "&encode=true";
            try {
    ...
    protected void dashidao_logout(HttpServletRequest request, HttpServletResponse response, Map<String, String> args){
        String url = CommUtil.getURL(request) + "/dashidao_logout.htm";
        try {
            response.sendRedirect(url);
        } catch (IOException e){
            e.printStackTrace();
    ...
    protected void dashidao_update_pws(HttpServletRequest request, HttpServletResponse response, Map<String, String> args){
