...
public class StoreViewAction {
		...
		if (claims != null) {
			...
			if (store != null) {
				...
				if (StringUtils.isNotEmpty(store_banner)) {
					...
					accessory.setAddTime(new Date());
					accessory.setPath("upload/img");
					accessory.setName(store_banner);
					accessoryService.save(accessory);
	...
	@ResponseBody
	public void storegoodssave(HttpServletRequest request, HttpServletResponse response, String goods_id,String sid, String status)
			throws IOException {
		Claims claims = SignFilter.checkSign(request, response);
		if (claims != null) {
			String id = claims.getId();

			Store store = this.storeService.getObjById(Long.parseLong(request.getParameter("sid")));

			//Store store = this.storeService.getObjById(Long.parseLong(id));

			String storeid = sid;
			//Store store = this.storeService.getObjById(Long.parseLong(storeid));
...
//				}
				...
				storeGoods.setStore(this.storeService.getObjById(Long.parseLong(storeid)));
	 ...
	 * @throws IOException
	 */
	@RequestMapping({ "/wap/storegoodsbyid.htm" })
	@ResponseBody
	public void storegoodsbyid(HttpServletRequest request, HttpServletResponse response, String goods_id, String sid,
			String currentPage,String state) throws IOException {

		Map params = new HashMap();
		StoreGoodsQueryObject gcqo = new StoreGoodsQueryObject(currentPage, params, "addTime", "desc");
		gcqo.addQuery("obj.store.id", new SysMap("store_id", Long.parseLong(sid)), "=");
		gcqo.addQuery("obj.goods.id", new SysMap("goods_id", Long.parseLong(goods_id)), "=");
		gcqo.addQuery("obj.state", new SysMap("state", Integer.parseInt(state)), "=");
		
		IPageList pList = this.storeGoodsService.list(gcqo);
		String flag = (pList.getResult()==null)+"";
		System.out.println();
		
		SignFilter.printNoCheck(request, response, flag);
	}
