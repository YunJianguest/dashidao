...
import com.dashidao.core.tools.*;
import com.dashidao.core.tools.bean.WxOauth2Token;
import com.dashidao.core.tools.bean.WxToken;
import com.dashidao.foundation.domain.*;
import com.dashidao.foundation.domain.query.AddressQueryObject;
import com.dashidao.foundation.domain.query.CouponInfoQueryObject;
import com.dashidao.foundation.domain.query.CouponQueryObject;
...
//                    mv.addObject("pay_session", pay_session);
     ...
     * @throws IOException
     */
    @RequestMapping({ "/wap/youHuiQuan_list.htm" })
    @ResponseBody
    public void youHuiQuan_list(HttpServletRequest request, HttpServletResponse response,String currentPage,String status) throws IOException 
    {
    	Map params = new HashMap();
    	CouponInfoQueryObject qo= new CouponInfoQueryObject(currentPage, params,"addTime", "desc");
    	Claims claims = SignFilter.checkSign(request, response);
		if (claims != null) {
			String id = claims.getId();
			qo.addQuery("obj.user.id", new SysMap("user_id", Long.parseLong(id)), "=");
			int zt = Integer.parseInt(status);
			if(zt ==0) 
			{
				qo.addQuery("obj.status", new SysMap("status",0), "=");
				qo.addQuery("obj.coupon.coupon_end_time", new SysMap("coupon_end_time", new Date()), ">=");//未过期
			}else if(zt ==1)
			{
				qo.addQuery("obj.status", new SysMap("status",0), "=");
				qo.addQuery("obj.coupon.coupon_end_time", new SysMap("coupon_end_time", new Date()), "<");//过期
			}
			IPageList pList = this.couponInfoService.list(qo);
			JsonConfig jsonConfig = new JsonConfig();
			jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessor("yy/MM/dd hh:mm"));
			// 需要过滤的类 + 属性
			...
			set.add("id");
			set.add("addTime");
			includeMap.put(CouponInfo.class, set);
