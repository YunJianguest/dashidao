...
import com.dashidao.foundation.domain.query.ChattingLogQueryObject;
import com.dashidao.foundation.domain.query.ChattingQueryObject;
...
import com.dashidao.manage.admin.tools.UserTools;
...
public class ChattingViewAction {
     ...
     * @throws IOException
     */
    @RequestMapping({ "/wap/chattingList.htm" })
    @ResponseBody
    public void chattingList(HttpServletRequest request, HttpServletResponse response,String currentPage,String type) throws IOException 
    {
    	Map params = new HashMap();
		...
		if (claims != null) 
		{
			...
			ChattingQueryObject qo = new ChattingQueryObject(currentPage, params,"addTime", "desc");
			qo.addQuery("obj.user2.id", new SysMap("user_id",Long.parseLong(id)), "=");
			qo.addQuery("obj.type", new SysMap("type",Integer.parseInt(type)), "=");
			IPageList pList = this.chattingService.list(qo);
			
			JsonConfig jsonConfig = new JsonConfig();
			jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessor("MM月dd日  hh:mm"));
			// 需要过滤的类 + 属性
			...
			set.add("addTime");
			set.add("logs");
			includeMap.put(Chatting.class, set);
			
			set = new HashSet<String>();
			set.add("content");
			includeMap.put(ChattingLog.class, set);
