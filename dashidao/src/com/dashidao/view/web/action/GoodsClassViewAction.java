...
public class GoodsClassViewAction {
   ...
   //商品列表
    ...
    @ResponseBody
    public void allgoods(HttpServletRequest request, HttpServletResponse response,String currentPage,String classid,String goodsSouSuo) throws IOException{
    	Map params = new HashMap();
    	...
    	//商品条件搜索
    	if(goodsSouSuo!=null && !"".equals(goodsSouSuo)) 
    	{
    		/**
    		 * 商品名称、分类名称
    		 */
    		gcqo.addQuery("obj.goods_name",
                    new SysMap("goods_name",
                    		"%"+goodsSouSuo+"%"), "like");
    		gcqo.addQuery("obj.gc.className", new SysMap("goods_name","%"+goodsSouSuo+"%"), "like", "or");
    	}
  ...
  //根据一级分类查询商品列表
    @RequestMapping({ "/wap/firstclassallgoods.htm" })
    @ResponseBody
    public void firstclassallgoods(HttpServletRequest request, HttpServletResponse response,String currentPage,String classid,String goodsSouSuo) throws IOException{
    	Map params = new HashMap();
    	System.out.println(classid);
    	GoodsQueryObject gcqo =new GoodsQueryObject(currentPage, params,
                "addTime", "desc"); 
    	if(classid!=null && !"".equals(classid)) 
    	{
    		gcqo.addQuery("obj.gc.parent.id",
                    new SysMap("gc_id",
                   		 Long.parseLong(classid)), "="); 
    	}
    	//商品条件搜索
  ...
  //店铺推荐商品
    ...
    //查询质量好的数量
    @RequestMapping({ "/wap/select_zhiLiangHao.htm" })
    @ResponseBody
    public void select_zhiLiangHao(HttpServletRequest request, HttpServletResponse response) throws IOException{
    	Map params = new HashMap();
    	
    	List pList = this.goodsService.query(
				" select count(a.fenshu) from OrderCom a where a.shangpinglun like '%1%' ",
				params, -1, -1);
        JsonConfig jsonConfig = new JsonConfig();
		jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessor("yyyy-MM-dd hh:mm:ss"));
		// 需要过滤的类 + 属性
  ...
  //查询包装精美的数量
    @RequestMapping({ "/wap/select_baozhuangjingmei.htm" })
    @ResponseBody
    public void select_baozhuangjingmei(HttpServletRequest request, HttpServletResponse response) throws IOException{
    	Map params = new HashMap();
    	
    	List pList = this.goodsService.query(
				" select count(a.fenshu) from OrderCom a where a.wuliupinglun like '%1%' ",
				params, -1, -1);
        JsonConfig jsonConfig = new JsonConfig();
		jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessor("yyyy-MM-dd hh:mm:ss"));
		// 需要过滤的类 + 属性
  ...
  //查询包装精美的数量
    @RequestMapping({ "/wap/select_fuwutaidu.htm" })
    @ResponseBody
    public void select_fuwutaidu(HttpServletRequest request, HttpServletResponse response) throws IOException{
    	Map params = new HashMap();
    	
    	List pList = this.goodsService.query(
				" select count(a.fenshu) from OrderCom a where a.fuwupinglun like '%1%' ",
				params, -1, -1);
        JsonConfig jsonConfig = new JsonConfig();
		jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessor("yyyy-MM-dd hh:mm:ss"));
		// 需要过滤的类 + 属性
