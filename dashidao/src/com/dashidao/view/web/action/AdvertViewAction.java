...
import com.dashidao.foundation.domain.AdvertPosition;
...
import com.dashidao.foundation.service.IAdvertPositionService;
...
public class AdvertViewAction {
     ...
     */
 	...
 	public void advert(HttpServletRequest request, HttpServletResponse response) throws IOException {
 		...
 		//List gg = this.advertService.query("select obj from Advert obj where obj.ad_ap.ap_type=:ap_type order by obj.addTime asc", params, 0, 4);
 		AdvertQueryObject ofqo = new AdvertQueryObject(currentPage,params,"addTime", "desc");
 		ofqo.addQuery("obj.ad_ap.ap_type", new SysMap("ad_ap_id", "wapslide"), "=");
 		ofqo.addQuery("obj.ad_end_time", new SysMap("ad_end_time", new Date()), ">=");
 		ofqo.addQuery("obj.ad_begin_time", new SysMap("ad_begin_time", new Date()), "<=");
 		IPageList pList = this.advertService.list(ofqo);
		...
		set.add("ad_url");
		set.add("ad_acc");
		includeMap.put(Advert.class, set);
     ...
     */
 	...
 	@RequestMapping({ "/wap/advertdetail.htm" })
 	@ResponseBody
 	public void advertdetail(HttpServletRequest request, HttpServletResponse response,String aid) throws IOException {
 	 
 	 
 		Advert advert=advertService.getObjById(Long.parseLong(aid));
 	    JsonConfig jsonConfig = new JsonConfig();
		jsonConfig.registerJsonValueProcessor(Date.class, new DateJsonValueProcessor("yyyy-MM-dd hh:mm:ss"));
		// 需要过滤的类 + 属性
		Map<Class<?>, Set<String>> includeMap = new HashMap<Class<?>, Set<String>>();
		Set<String> set = new HashSet<String>(); 
		set.add("ad_title");
		set.add("ad_begin_time");
		set.add("ad_end_time");
		set.add("ad_status");
		set.add("ad_text");
		set.add("ad_url");
		set.add("ad_click_num");
		set.add("ad_acc"); 
		includeMap.put(Advert.class, set);
		set=new  HashSet<String>(); 
		set.add("name");
		...
		JSONArray json = JSONArray.fromObject(advert, jsonConfig);
		SignFilter.print(request, response, json);
