...
public class AddressViewAction {
	...
	@ResponseBody
	public void address(HttpServletRequest request, HttpServletResponse response,String addRessSouSuo) throws IOException {
		...
		if (claims != null) {
			...
			if(addRessSouSuo!=null && !"".equals(addRessSouSuo)) 
			{
				/**
	    		 * 收货人姓名 电话模糊查询
	    		 */
				ofqo.addQuery("obj.telephone",
	                    new SysMap("telephone",
	                    		addRessSouSuo+"%"), "like");
				ofqo.addQuery("obj.trueName", new SysMap("trueName",addRessSouSuo+"%"), "like", "or");
				ofqo.addQuery("obj.mobile", new SysMap("mobile",addRessSouSuo+"%"), "like", "or");
			}
	 ...
	 * @throws IOException
	 */
	@RequestMapping({ "/poster/areall.htm" })
	@ResponseBody
	public void  areall1(HttpServletRequest request, HttpServletResponse response,String level) throws IOException {
		Map params = new HashMap(); 
 		if (StringUtils.isEmpty(level)) {
			level="3";
		}
