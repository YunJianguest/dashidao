...
import com.dashidao.foundation.domain.WaterMark;
 ...
 */
@Controller
public class WaterMarkPosterAction {
    @Autowired
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "图片水印", value = "/poster/watermark.htm*", rtype = "poster", rname = "图片管理", rcode = "album_poster", rgroup = "其他设置")
    @RequestMapping({"/poster/watermark.htm"})
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "图片水印保存", value = "/poster/watermark_save.htm*", rtype = "poster", rname = "图片管理", rcode = "album_poster", rgroup = "其他设置")
    @RequestMapping({"/poster/watermark_save.htm"})
    public ModelAndView watermark_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage, String cmd){
        ...
        mv.addObject("url", CommUtil.getURL(request) + "/poster/watermark.htm");
