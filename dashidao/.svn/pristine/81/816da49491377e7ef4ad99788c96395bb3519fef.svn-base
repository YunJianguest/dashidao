...
import com.dashidao.foundation.domain.query.StackGoodsQueryObject;
 ...
 */
@Controller
public class StackGoodsManageAction {
    @Autowired
    ...
    @Autowired
    private IStackGoodsService stackGoodsService;
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "商品列表", value = "/admin/stack_goods_list.htm*", rtype = "admin", rname = "商品管理", rcode = "admin_goods", rgroup = "商品")
    @RequestMapping({"/admin/stack_goods_list.htm"})
    public ModelAndView stack_goods_list(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType){
        ModelAndView mv = new JModelAndView("admin/blue/stack_goods_list.html",
        ...
        CommUtil.saveIPageList2ModelAndView(url + "/admin/stack_goods_list.htm", "",
                                            params, pList, mv);
   ...
   @SecurityMapping(display = false, rsequence = 0, title = "上架商品列表", value = "/admin/goods_tostack.htm*", rtype = "admin", rname = "商品管理", rcode = "admin_goods", rgroup = "商品")
    @RequestMapping({"/admin/goods_tostack.htm"})
    public ModelAndView goods_outline(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType){
        ModelAndView mv = new JModelAndView("admin/blue/goods_tostack.html",
                                            this.configService.getSysConfig(), this.userConfigService
                                            .getUserConfig(), 0, request, response);
        String url = this.configService.getSysConfig().getAddress();
        if ((url == null) || (url.equals(""))){
        ...
        String params = "";
        StackGoodsQueryObject qo = new StackGoodsQueryObject(currentPage, mv, orderBy,
                orderType);
        WebForm wf = new WebForm();
        wf.toQueryPo(request, qo, StackGoods.class, mv);
        //qo.addQuery("obj.goods_status", new SysMap("status", Integer.valueOf(-2)), "=");
        IPageList pList = this.stackGoodsService.list(qo);
        CommUtil.saveIPageList2ModelAndView(url + "/admin/stack_goods_list.htm", "",
                                            params, pList, mv);
        List gbs = this.goodsBrandService.query(
                       "select obj from GoodsBrand obj order by obj.sequence asc",
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "商品添加上架", value = "/admin/stackgoods_add.htm*", rtype = "admin", rname = "商品管理", rcode = "admin_goods", rgroup = "商品")
    @RequestMapping({"/admin/stackgoods_add.htm"})
    public ModelAndView goods_add(HttpServletRequest request, HttpServletResponse response, String currentPage){
        ModelAndView mv = new JModelAndView("admin/blue/goods_tostack.html",
                                            this.configService.getSysConfig(), this.userConfigService
                                            .getUserConfig(), 0, request, response);
     
    Goods goods=goodsService.getObjById(Long.parseLong(request.getParameter("id")));
    StackGoods stackgoods=new StackGoods();
   // stackgoods.setId(goods.getId());
    stackgoods.setGoods_name(goods.getGoods_name());
    stackgoods.setGoods_price(goods.getGoods_price());
    stackgoods.setGoods_code(goods.getGoods_serial());
    stackgoods.setGoods_nums(1);
    stackgoods.setStatus(0);
    //获取当前登录用户
    SecurityContext ctx = SecurityContextHolder.getContext();  
    Authentication auth = ctx.getAuthentication();  
    User user = (User) auth.getPrincipal(); 
    stackgoods.setStack_id(user.getId());
    stackgoods.setTrader_id(goods.getGoods_store().getId());
    stackGoodsService.save(stackgoods);
    ...
    public ModelAndView goods_edit(HttpServletRequest request, HttpServletResponse response, String id, String currentPage){
        ...
        if ((id != null) && (!id.equals(""))){
            Goods goods = this.StackGoodsService.getObjById(Long.valueOf(Long.parseLong(id)));
    ...
    public ModelAndView goods_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage, String cmd, String list_url, String add_url){
        ...
        if (id.equals("")){
            ...
            Goods obj = this.StackGoodsService.getObjById(Long.valueOf(Long.parseLong(id)));
            goods = (Goods)wf.toPo(request, obj);
        }
        if (id.equals(""))
            this.StackGoodsService.save(goods);
        else
            this.StackGoodsService.update(goods);
    ...
    public String goods_del(HttpServletRequest request, String mulitId) throws Exception {
        ...
        for (String id : ids){
            if (!id.equals("")){
                Goods goods = this.StackGoodsService.getObjById(
                ...
                this.StackGoodsService.delete(goods.getId());
    ...
    public void ajax(HttpServletRequest request, HttpServletResponse response, String id, String fieldName, String value) throws ClassNotFoundException {
        ...
        this.StackGoodsService.update(obj);
