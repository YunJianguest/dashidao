...
import com.dashidao.core.annotation.Log;
...
import com.dashidao.foundation.domain.LogType;
import com.dashidao.foundation.domain.StoreStat;
...
import com.dashidao.foundation.service.IStoreStatService;
...
import com.dashidao.manage.admin.tools.StatTools;
...
public class BaseManageAction {
    ...
    throws IOException {
        ...
        if (this.configService.getSysConfig().isSecond_domain_open()){
            Cookie dashidao_user_session = new Cookie("dashidao_user_session", user.getId().toString());
            dashidao_user_session.setDomain(CommUtil.generic_domain(request));
            response.addCookie(dashidao_user_session);
        }
    ...
    public void logout_success(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (this.configService.getSysConfig().isSecond_domain_open()){
            ...
            for (Cookie cookie : cookies){
                if (cookie.getName().equals("dashidao_user_session")){
                    cookie.setMaxAge(0);
                    cookie.setValue("");
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "顶部广告管理", value = "/admin/top_advert.htm*", rtype = "admin", rname = "顶部广告管理", rcode = "top_advert_seller", rgroup = "基础管理")
    @RequestMapping({"/admin/top_advert.htm"})
    public ModelAndView top_advert(HttpServletRequest request, HttpServletResponse response){
        ModelAndView mv = new JModelAndView(
            "admin/blue/top_advert.html", this.configService
            .getSysConfig(),
            this.userConfigService.getUserConfig(), 0, request, response);
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "顶部广告管理修改", value = "/admin/top_advert_update.htm*", rtype = "admin", rname = "顶部广告管理", rcode = "top_advert_update_seller", rgroup = "基础管理")
    @RequestMapping({"/admin/top_advert_update.htm"})
    public ModelAndView top_advert_update(HttpServletRequest request, HttpServletResponse response){
        ModelAndView mv = new JModelAndView(
            "admin/blue/top_advert_update.html", this.configService
            .getSysConfig(),
            this.userConfigService.getUserConfig(), 0, request, response);
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "顶部广告管理添加", value = "/admin/top_advert_insert.htm*", rtype = "admin", rname = "顶部广告管理", rcode = "top_advert_insert_seller", rgroup = "基础管理")
    @RequestMapping({"/admin/top_advert_insert.htm"})
    public ModelAndView top_advert_insert(HttpServletRequest request, HttpServletResponse response){
        ModelAndView mv = new JModelAndView(
            "admin/blue/top_advert_insert.html", this.configService
            .getSysConfig(),
            this.userConfigService.getUserConfig(), 0, request, response);
