...
public class ArticleClassManageAction {
    ...
    //文章内容预览
    @SecurityMapping(display = false, rsequence = 0, title = "文章下级分类", value = "/admin/articleclass_show.htm*", rtype = "admin", rname = "文章分类", rcode = "article_class", rgroup = "网站")
    @RequestMapping({"/admin/articleclass_show.htm"})
    public ModelAndView articleclass_show(HttpServletRequest request, HttpServletResponse response, String pid, String currentPage){
        ModelAndView mv = new JModelAndView(
            "admin/blue/articleclass_show.html", this.configService
            .getSysConfig(),
            this.userConfigService.getUserConfig(), 0, request, response);
