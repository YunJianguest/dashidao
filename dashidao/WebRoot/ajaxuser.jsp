...
6：自定义查询语句"table=dashidao_user&dates=loginDate,addTime,birthday,lastLoginDate&sql=SELECT * FROM `dashidao_user`where addTime  Between '"+getDay(-10000)+"' And '"+getDay(0)+"'",
//源码：--%>
...
//加密的字符串如何处理,我还没有想好.
   ...
   // String url="jdbc:mysql://192.168.1.115:3306/dashidao"; //test为数据库名称
    String url="jdbc:mysql://127.0.0.1:3306/dashidao"; //test为数据库名称
            ...
            case 3:
                ...
                while (rs.next()) {
                    ...
                    for (int i2 = 0; i2 < rsmd.getColumnCount(); i2++) {
                        ...
                        if(columnLable.indexOf("loginDate") != -1 || columnLable.indexOf("addTime") != -1 || columnLable.indexOf("birthday") != -1 || columnLable.indexOf("lastLoginDate") != -1){
                ...
                case 4:
                	...
                	if (request.getParameter("page_size") != null && request.getParameter("page_no") != null) {
                	 ...
                	 JSONObject jo3 = new JSONObject(); 
                      ...
                      //分页
                      if(bool && bool3) {
                          sql += " desc limit " + (Integer.parseInt(request.getParameter("page_no")) - 1) * Integer.parseInt(request.getParameter("page_size")) + "," + request.getParameter("page_size");
                      }
                      System.out.println(sql);
                      pst = conn.prepareStatement(sql);
                      rs = pst.executeQuery();
                      ...
                      while (rs.next()) {  
                          ...
                          for (int i2 = 0; i2 < rsmd.getColumnCount(); i2++) {
                              String columnLable = rsmd.getColumnLabel(i2 + 1); 
                             // System.out.print(columnLable.toString()); 
                             // System.out.print(rs.getObject(i2 + 1));
                              //System.out.print(rs.getObject(i2 + 1).toString().substring(0,19));
                              //System.out.print(columnLable);
                              String str=null;
                              if(dat!=null){
                      ...
                      if(request.getParameter("page_size") == null){
                          ...
                          pageSize2=request.getParameter("page_size");
                      }
                      if(request.getParameter("page_no") == null){
                          pageNo2="1";
                      }else{
                          pageNo2=request.getParameter("page_no");
                      }
                      System.out.println(json.size());

                      System.out.println(Integer.parseInt(pageSize2));
                      System.out.println(Integer.parseInt(pageNo2));
                      System.out.println((int)Math.ceil(new Double(json.size())/Integer.parseInt(pageSize2))); 
                      if(bool&&bool3){
                    	  jo.put("per_page",Integer.parseInt(pageSize2));
                          jo.put("current_page",Integer.parseInt(pageNo2));
                          jo.put("last_page",(int)Math.ceil(new Double(toal1)/Integer.parseInt(pageSize2)));
                      } 
                      jo.put("total",toal1);
                      jo.put("data",json);
                      jsonAll.put("data",jo);
